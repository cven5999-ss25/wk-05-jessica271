...
1.  What's the `tidyverse` Package? Describe in maximum two sentences below.
    ...
    \# It’s a collection of R packages that provide functions for data analysis, including data visualization and manipulation.
...
2.  What's the [`here` Package](https://here.r-lib.org/articles/here.html)? Why is useful when working with quarto or R Markdown files?
    ...
    Here package enables easy file referencing especially when importing data from a file.
...
levels_income_cat <- c("low income",
  "lower-middle income",
  "upper-middle income",
  "high income")
...
2.  Starting with the `waste_data_long_mean` object, execute the code and observe the output in the Console. Would you publish this table in this format in a report? No

3.  Starting with the `waste_data_long_mean` object, use the pipe operator to add another line of code which uses the `pivot_wider()` function to bring the data from a long format into a wide format using names for variables from `waste_category` and corresponding values from `mean_percent`

4.  Execute the code and observe the output in the Console. Would you publish this table in a report in this format? Yes
